...
int np_shell() { //np_shell()
    ...
    while (true) {
        ...
        // string line_trim = line.substr(0, line.length() - 1); // trim off the last character '\0'
         ...
         // trim command
        string whitespace ("\n\r");
        size_t found = line.find_last_of(whitespace);
        while (found != string::npos) {
            line.erase(found, string::npos);
            found = line.find_last_of(whitespace);
        }

        while (line.back() == ' ') { // 若字串尾巴有空白就會刪掉
            line.pop_back();
        }
        if (DEBUG) { 
            cout << "trimmed command: " << line << ": <- test next line" << endl;
        }
