    ...
    if (command.size() > 0) {
        ...
        for (int i = 0; i < command.size(); i++) {
            ...
            switch (expression) {
                case -4: // single command ends with '|' or '!'
                    if (user_pipe_in == 1) {
                        ...
                        if ((user_pipe_in_idx <= 30) && user_table_flag[user_pipe_in_idx - 1] == 1) {
                            ...
                            for (int k = 0; k < MAX_PIPE; k++) {
                                if (num_pipe_table[k][3] == sender_socket && num_pipe_table[k][4] == receiver_socket) {
                ...
                case -2: // single command without numpipe end // TODO 修好一種：其他種複製貼上><
                    if (user_pipe_in == 1) {
                        temp_check = 1;
                        user_pipe_exist = 0;
                        receiver_socket = current_socket;
                    ...
                    if (user_pipe_out == 1) {
                        ...
                        if ( user_pipe_out_idx <= 30 && user_table_flag[user_pipe_out_idx - 1] == 1) {
                            ...
                            for (int k = 0; k < MAX_PIPE; k++) {
                                if (num_pipe_table[k][3] == sender_socket && num_pipe_table[k][4] == receiver_socket) {
                                    usrpipe_r_fd = num_pipe_table[k][0];
                                    temp_w_fd = num_pipe_table[k][1];
